const SITE_URL = 'https://findawaytoplay.vercel.app';
...
async function shareCard(btn) {
  ...
  if (navigator.share) {
    try { await navigator.share({ title: data.title, text: data.text, url: SITE_URL }); return; } catch (e) {}
  }
...
async function sharePage() {
  if (navigator.share) {
    try {
      await navigator.share({ title: 'Find a Way to Play — Sonic Remedy | Soozhee', text: 'Find a Way to Play by Soozhee. Sonic Remedy : Audiosensory Resonance | Trust Your Path.', url: SITE_URL });
      return;
    } catch (e) {}
  }
  await navigator.clipboard.writeText(SITE_URL);
